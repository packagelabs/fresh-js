...
import { Project, OnChainBlindMinter, schema, OnChainMinter } from './index';
...
describe('OnChainBlindMinter', () => {
  ...
  const minterAuthorizer = new Authorizer({ address: '0xf8d6e0586b0a20c7', keyIndex: 0, signer });

  const payerAuthorizer = new Authorizer({ address: '0xf8d6e0586b0a20c7', keyIndex: 0, signer });
  ...
  const project = new Project({
    ...
    authorizers: {
      minter: minterAuthorizer,
      payer: payerAuthorizer,
      proposer: payerAuthorizer,
    },
  });

  const publicKey = privateKey.getPublicKey();

  await collection.deployContract(publicKey, HashAlgorithm.SHA3_256);
  ...
  it('should work', async () => {
    // const sale2 = new ClaimSale(project);

    // await sale2.stop();

    // return;

    await minter.deployContract(publicKey, HashAlgorithm.SHA3_256);
    ...
    const nftID = await sale.claimNFT(authorizer.address, authorizer);
  ...
  // it('should generate a ClaimSale contract', async () => {
  //   console.log(await sale.getContract());
  // });
